...
\citet{kauhanen2025canaries} use comparable population-level register data from Finland (the Incomes Register) and find no statistically significant effect of generative AI on employment or wages. Their synthetic difference-in-differences design compares the most exposed occupations (top~10\% on the C-AIOE index of Pizzinelli et al.~2023) to the least exposed (bottom~10\%) at the occupation level. This aggregation averages over all employers within each occupation, and may therefore not capture within-employer composition shifts of the kind our employer-level specification (Section~\ref{sec:employer_did}) is designed to identify. Our employer~$\times$~quartile and employer~$\times$~month fixed effects absorb all time-varying employer-level shocks, isolating within-firm recomposition across AI exposure quartiles. The two designs are complementary: the Finnish occupation-level null and our employer-level age gradient are not necessarily inconsistent, since aggregate occupation-level effects may be zero even if the composition of employment within employers is shifting. Whether Finland exhibits similar within-employer patterns remains an open empirical question.
