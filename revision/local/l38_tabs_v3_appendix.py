...
def size_reliability() -> None:
    ...
    note = (
        ...
        rf"{float(v['variance_within_firm']):.0f}. It is reported and never "
        r"used to correct an estimate: the measurement error here is not "
        r"classical, and the answer to a low reliability is a design that "
        r"raises it. Panel B. Poisson on the specification of Equation~(2), "
        ...
        r"two size groups are the thirds of the incumbent count as the export "
        r"cut them; the count is skewed enough that the lower cut point falls "
        r"at one incumbent, so the first group is empty and the split is the "
        r"two-way one shown."
    )
    tex += [r"\bottomrule", r"\end{tabular}",
            r"\begin{minipage}{0.92\textwidth}\footnotesize\vspace{4pt}",
