#!/usr/bin/env python3
"""
79_last_gaps.py -- the three gaps the 22 September vetting round left open
                   after script 78: inference by industry for the sexes,
                   the industry-and-cycle fit at 26 to 30, and a count of
                   the payslips the panel never sees.
...
======================================================================
  ...
  CANARIES_79_OUT (default output_79); the lane runner sets both.
  Budget: A two to three hours and the heaviest thing here (one Poisson
  fit on the 40.5 million-row sex panel, two if lane 25's employer-
...
A. THE SEXES, CLUSTERED BY INDUSTRY. Script 78's Part C showed that the
   ...
   The employer-clustered standard errors beside them are lane 25's own
...
C. THE UNCOUNTED PAYSLIPS. Every count in this paper comes from a worker
   ...
   artefact, and that is the editor's mechanism stated as a testable
   ...
   is higher in exposed firms at young ages is the editor's mechanism and
...
IN THE PAPER. A replaces the standard errors on the sex sentence of
Section 3 and Table 1, wherever the industry-clustered ones are quoted
beside the employer-clustered ones. B goes beside 78's Part F in Online
Appendix III.2, where the industry test is reported by band. C is a
paragraph of Section 2 or Part IV of the online appendix on what the
employer declarations contain that the panel does not, and it is the
answer to the editor's coverage objection at the level of the individual
payslip.
"""
...
# A reads lane 25's employer-clustered export from whichever folder that
# lane wrote. 25b ran parts BC, the full runner writes output_78; the
# others are listed so a differently split submission is still found.
LANE25_ROOT = HERE
...
def lane25_gender(terms: list):
    """
    Lane 25's employer-clustered sex fit, if it reached the share.
...
def probe_declaration_age(conn, years) -> tuple:
    ...
    change and a guessed column name is how three MONA rounds were lost in
    September.
