...
def main():
    ...
    for label, outcome in (("stock", "n_emp"), ("hires", "n_hire")):
        ...
        for key, e in expos.items():
            ...
            if key == "daioe" and "telework" in expos:
                got = opt(f"horse race {label}", horserace, bal,
                          expos["telework"], l47, f"hr63_{label}")
                r, ncell = got if got is not None else (None, 0)
                if r is None or r.empty:
                    FAILURES.append(f"horserace/{label}")
                else:
                    for _, x in r.iterrows():
                        hr_rows.append({"outcome": label,
                                        "spec": str(x["spec"]),
                                        "term": x["term"],
                                        "coef": float(x["coef"]),
                                        "se": float(x["se"]),
                    ...
                    pd.DataFrame(hr_rows).to_csv(OUT / "horserace.csv",
                                                 index=False)
                    print(f"    horse race, {label}, both measures at once:")
                    for h in hr_rows:
                        if (h["outcome"] != label
    ...
    if rows:
        ...
        for outcome in sorted(G["outcome"].unique()):
            piv = (G[G.outcome == outcome]
                   .pivot_table(index="age_group", columns="measure",
                                values="coef"))
            order = [a for a in l47.AGES if a in piv.index]
            lines += [f"age gradient, outcome = {outcome}:",
    ...
    if hr_rows:
        ...
        for _, h in pooled.iterrows():
            ...
            lines.append(f"    {h['outcome']:<6} {nm:<9} {h['coef']:+.4f} "
                         f"(SE {h['se']:.4f}) t "
                         f"{h['coef']/max(h['se'],1e-12):+.2f}")
