#!/usr/bin/env python3
"""
84_occupation_route_path.py -- the quarterly path of Figure 3, on lane
...
======================================================================
  ...
  the lane runners set both. No database connection is needed once lane
  28a has cached the cascade and the 2019 counts, which it has.
======================================================================

QUESTION
Lanes 28 and 29 moved every register estimate in the paper onto a firm
score built from the employer's own 2019 occupation mix. ONE EXHIBIT DID
NOT MOVE: Figure 3, the quarterly path that dates the step, is still
drawn from script 68's education-route fit. The paper therefore asks a
reader to take Table 1 on one definition of the treatment and its timing
evidence on another. The caption says so, which keeps the draft honest,
but a referee will ask why the dating was the one thing left behind.
...
as in lane 29. The cascade pull behind it is cached, so this lane reads
...
the specification the pooled estimates come from, and the whole point of
the figure is that it decomposes the same fit. The Riksbank interaction
stays on in every path fit, so each path coefficient is a step from the
level of the tightening months, exactly as in the education-route
figure; the figure's caption states that reading and it does not change.
...
WHAT IS ESTIMATED
  ...
  M  the monthly path at both bands, every month from December 2022
     ...
     fits. This is the diagnostic of Online Appendix III.2 and it
     carries NO verdict: the calendar terms are at quarter frequency in
     the reported specification, so a monthly coefficient retains
     whatever separates the month from its own quarter's mean, and at
     22-25 that residual is large.

WHY NOT THE YEAR SHAPE. Script 68 also fits a yearly path. No exhibit
draws it and the paper quotes no number from it, so it is not run here;
a lane that fits what nothing reads spends an hour to produce a column
that will be filed and never opened.
...
that has already been made and reported, not a new estimate of it.
  ...
  1. THE DATING REPRODUCES at 22-25 if no quarter before 2024 carries a
     ...
     AND at least one quarter from 2024 onward does. The education route
     is flat through 2023Q3, dips in 2023Q4 and opens its shortfall in
     the second half of 2024; the rule is written on the year and not on
     the quarter, because a rule that named 2024Q3 would be read off the
     figure it is supposed to judge.
  2. THE LAG REPRODUCES if the first quarter meeting that description at
     ...
     later; if the two open together, or the older one first, that
     sentence goes.
  3. The monthly path carries NO verdict and is exported for the
     appendix diagnostic alone.
  Neither rule is a condition for quoting anything else. Lane 28's
  pooled estimates stand whatever the path does; what the rules decide
  is which sentences about TIMING survive.

A CONSISTENCY LINE, REPORTED AND NOT GATED
The unweighted mean of the quarter coefficients from 2024 onward is
printed beside lane 28b's pooled adoption step of -0.0578. The two are
...
(script 47L, cached by lane 28a) and L_counts_2021 to 2025 (script 47L).
Performs no SQL of its own; if the cascade is absent it is pulled
through 82's own query, which is the only database work this lane can
do.
...
IN THE PAPER
Figure 3 of main_v3.tex, and the monthly diagnostic of Online
Appendix III.2.
"""
...
# Lane 28b's pooled adoption step at 22-25 on this same score, for the
# consistency line. Not a gate.
POOLED_22 = -0.0578
# The education route's own dating, as the paper states it. The path
# numbers behind it are in the lane 14 export and are not copied here;
...
def consistency(rows: list) -> str:
    ...
    lane 28b's pooled step. Reported, never gated."""
...
def main():
    ...
    # THE SCORE. Lane 28's, built in lane 28's own function, not rebuilt.
