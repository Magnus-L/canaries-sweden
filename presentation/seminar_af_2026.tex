...
\setbeamertemplate{footline}{%
  ...
  \hbox{%
    \begin{beamercolorbox}[wd=.4\paperwidth,ht=2.5ex,dp=1ex,left]{footline}%
      \hspace*{2mm}\footnotesize Lodefalk --- Arbetsförmedlingen Seminar
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.3\paperwidth,ht=2.5ex,dp=1ex,center]{footline}%
      \footnotesize 26 March 2026
    \end{beamercolorbox}%
...
\date{Arbetsförmedlingen --- Analysavdelningen\\26 March 2026}
...
\vspace{5mm}
\begin{center}
\textit{Next two slides: how we measure AI exposure, and why Swedish register data matters}
\end{center}
\end{frame}
...
% ============================================================
% IDENTIFICATION: POSTING MARGIN
% ============================================================
\begin{frame}{Identification: posting margin}
\textbf{Baseline DiD with two treatment periods:}
\vspace{-2mm}
\begin{equation*}
\ln V_{o,t} = \alpha_o + \alpha_t + \beta_1 (\text{PostRB}_t \times \text{High}_o) + \beta_2 (\text{PostGPT}_t \times \text{High}_o) + \varepsilon_{o,t}
\end{equation*}

\begin{itemize}
    \item $\alpha_o$: occupation FE; \quad $\alpha_t$: month FE
    \item PostRB: April 2022 (Riksbank first hike); \quad PostGPT: November 2022 (ChatGPT launch)
    \item High$_o$: top DAIOE quartile (Q4)
    \item SEs clustered at occupation level
\end{itemize}

\vspace{3mm}
\begin{block}{Timing test logic}
If $\beta_1 < 0$ and $\beta_2 \approx 0$: decline is monetary, not AI\\
If $\beta_1 \approx 0$ and $\beta_2 < 0$: decline is AI, not monetary\\
AI exposure is uncorrelated with interest rate sensitivity ($r = 0.04$, $p = 0.51$)
\end{block}
\end{frame}
...
% ============================================================
% EVENT STUDY PANEL (ALL AGE GROUPS)
% ============================================================
...
\begin{itemize}
    ...
    \item Oaxaca-Blinder reweighting: occupational composition explains $\sim$1/3 of the gap
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Posting margin}
\begin{itemize}
    \item 8 specifications: negative in 7/8, significant in 2
    \item Alternative AI measure (Eloundou et al.\ 2024, GPT-specific): timing shifts to ChatGPT period --- consistent with measure capturing GenAI vs broad AI
    \item 85\% quartile agreement between measures
    \item Interest rate sensitivity uncorrelated with AI exposure ($r = 0.04$)
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Employment margin}
\begin{itemize}
    \item Placebo dates: July 2022, November 2021 --- no effect
    \item Extensive margin: zero-cell share for 22--25 widens from 8.9pp to 13.1pp {\tiny\hyperlink{backup:extensive}{\beamergotobutton{Fig}}}
    \item Spotlight occupations: software devs, customer service, payroll admin, receptionists --- all show age gradient
    \item SSYK match robustness (cascading lookup)
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
% CAVEATS
% ============================================================
\begin{frame}{Some caveats and limitations, e.g.,:}
\begin{enumerate}
