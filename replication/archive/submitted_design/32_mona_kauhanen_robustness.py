#!/usr/bin/env python3
"""
32_mona_kauhanen_robustness.py: the submitted version's employer design, staged
against the Finnish specification, and the non-match series of Online Appendix VI.1.

Runs inside Statistics Sweden's MONA environment (project P1207). It is kept in the
archive because the online appendix quotes two of its outputs; the design itself is
withdrawn (Online Appendix Part IV).

WHAT IT ESTIMATES
  Poisson pseudo-maximum likelihood on employer x exposure-bin x month counts of
  workers in one age band, with employer-by-bin and employer-by-month fixed effects,
  standard errors clustered by employer, and two treatment terms, PostRB x High from
  April 2022 and PostGPT x High from December 2022. Each worker is classified by the
  occupation the annual register records for them in the year, carried back from
  the 2023, 2022 and 2021 registers from 2023 onwards: the construction that the
  as-of backtest of Online Appendix IV.3 shows the register's publication lag can
  bias. Five variants are run, one change at a time:
    1. employers with at least five person-months, DAIOE quartile (the submitted
       specification; its 22-25 coefficient, -0.174, is the "about -0.17" the paper
       quotes for a design that classifies workers by their own occupation);
    2. the size floors of Brynjolfsson et al. (2025), a mean of at least ten workers
       a month and a hundred person-months;
    3. those floors with the Eloundou et al. exposure cut into quintiles;
    4. specification 1 reweighted to Finland's 2022 occupation and industry mix
       (Eurostat LFS marginals, finland_marginals_2022.txt);
    5. specification 1 without ICT specialists (SSYK 25xx).

  A diagnostic then counts, year by year from 2019 to June 2025, the person-months
  in the employer declarations and those in the classified panel, and writes their
  difference as a share (attrition_yearly_totals.csv). Online Appendix VI.1 quotes
  this share, 9.2 to 10.5 per cent, as the overall non-match rate. Note the
  definition: the denominator counts distinct persons per month at every age; the
  numerator counts employer-person-months aged 22 to 69 with a code, so the share
  also includes workers outside those ages and counts a person once per employer.

READS  the monthly employer declarations (dbo.Arb_AGIIndivid{YYYYMM}_Def, 2019 to
       2024; _Prel, January to June 2025), LISA (dbo.Individ_2019 to _2023),
       Foretag_FDB for industry (variant 4), and on the project share the DAIOE
       quartiles, the Eloundou scores and crosswalks, and the Finnish marginals.
WRITES output_32/: step1_poisson_current.csv to step5_poisson_no_ict.csv,
       kauhanen_comparison.csv, kauhanen_summary.txt, attrition_yearly_totals.csv,
       attrition_by_quartile.csv, and a cache of the classified panel that stays on
       MONA. Every export is coefficients or counts; the quartile diagnostic sets
       cells below five to missing.
SOFTWARE Python with pandas, numpy and pyodbc; R with fixest 0.13.2, called through
       r_fepois.R, which must sit beside this script.
RUNTIME about two hours, most of it the first SQL pull.
"""
...
# --- R + fixest pre-flight (pyfixest is not available in MONA) ---
...
# The floors of Brynjolfsson et al. (2025): ">=10 employees per age group per
# month and >=100 cumulatively across exposure quantiles". Implemented as:
...
RUN_ALL_REGARDLESS = True   # set False to stop when variant 1 is null
...
# Brynjolfsson et al. (2025) exclude these occupations.
ICT_SSYK_PREFIX = "25"
...
def step1_poisson_current(panel_ssyk_age, daioe):
    ...
    Tests whether the switch from OLS on log(1 + count) to Poisson alone
...
def step2_poisson_threshold(panel_ssyk_age, daioe):
    ...
    Isolates the marginal effect of the threshold change.
    """
    print("\n" + "=" * 70)
...
def step5_poisson_no_ict(panel_ssyk_age, daioe):
    ...
    SSYK 25xx (four-digit codes starting with '25') are the ICT
    specialists of SSYK 2012, the counterpart of the SOC 15-1000 series
    that Brynjolfsson et al. (2025) exclude.
    """
    print("\n" + "=" * 70)
...
def attrition_diagnostic(panel_ssyk_age, daioe):
    """
    Year-by-year share of employer-declaration person-months without a
    classified record, and the quartile mix of those classified.
...
def write_comparison_table(step1, step2, step3, step4, step5):
    """
    Write a comparison CSV: gamma2 (PostGPT x High) for all
    (spec, age_group) combinations, side by side.
    """
    print("\n" + "=" * 70)
...
def write_prose_summary(step1, step2, step3, step4, step5):
    """
    Plain-text summary of the staged comparison.
    """
    ...
    # The 22-25 row across the variants.
    lines.append("=" * 60)
