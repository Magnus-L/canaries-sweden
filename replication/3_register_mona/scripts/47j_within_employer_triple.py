...
generative AI arrives? Exposure is measured on the employer, here from the
2019 education mix of its incumbents aged 31 to 69 (the education route),
and the within-employer variation comes from age, which is read from the
birth year and cannot go stale. No worker under 31 enters the exposure
measure, and no occupation code recorded after 2019 enters anything. The
paper reports the same within-employer design with the employer scored
from the 2019 occupations of its incumbents (script 82); this script
supplies the sample rules, fixed effects and incumbent bands that every
...
IN THE PAPER
Section 2: the fixed effects and sample rules of Equation (2) (an
employer holds the young band and at least one incumbent band; five
incumbent person-months as the scoring floor; quartiles weighted by
incumbent employment). MIN_FIRM_INCUMBENTS, INCUMBENT_BANDS, YOUNG_BANDS,
FES and _drop_dead_cells are imported by every later register script,
script 82 included. The coefficients in triple_estimates.csv, dated at
the launch on a panel ending in 2023, are not quoted.
"""
