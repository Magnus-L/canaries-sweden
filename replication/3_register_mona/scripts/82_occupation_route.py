#!/usr/bin/env python3
"""
82_occupation_route.py: the paper's employment estimates, on a firm score
built from the 2019 occupations of each employer's own incumbents.
...
======================================================================
  ...
  with CANARIES_82_OUT (default output_82); master.py sets both.
======================================================================

QUESTION
Inside the same employer, did the employment of young workers relative to
their older colleagues change differently at employers exposed to
generative AI? An employer is scored directly by the employment-weighted
mean DAIOE generative-AI percentile of the occupations its own incumbents
aged 31 to 69 held in 2019, and the top quartile of that score is the
treatment. The freeze year, the incumbent restriction, the person-month
floor and the quartile logic are those of script 47j; no education record
enters the score. An earlier construction routed the exposure through
the education of the incumbents (script 47h's score book); its estimates
are printed beside every estimate here as the education route, for
comparison only.
...
IN THE PAPER
This is the source of the paper's employment estimates. Part A: the
coverage of the score's codes (Section 2 and Online Appendix IV.4,
Table A28) and the sample description of Online Appendix I.2 (Table A2).
Part B: the adoption steps at 22-25 and 26-30 and the six-band profile
(Table 1, Figure 2's filled markers, Section 3). Part C: the sex rows,
hires and separations (Table 1, Section 3) and the vintage check (Online
Appendix IV.3, the note to Table A27). build_exposure is imported by
scripts 83 to 93, which put every other exhibit on the same score.
"""
...
# dictionary of the ORU-MICRO-AI database (the LISA individual tables).
#
...
# panel (daioe_panel_ssyk2012, year 2023, exp_change_genai ranked to a
# percentile) so the MONA figure has something to be checked against.
# Verified by recomputation: the shares and the
...
def cascade_sql(plan: dict) -> str:
    ...
    first and precision second, which is the order the design fixes;
    the alternative, preferring an older four-digit code to a
