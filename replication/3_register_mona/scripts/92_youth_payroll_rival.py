#!/usr/bin/env python3
"""
92_youth_payroll_rival.py: is the decline at 22-25 an artefact of the end
of the reduced youth payroll contribution in March 2023?
...
THE RIVAL
Sweden ran a reduced employer social contribution for young workers,
roughly 19.73 per cent against the standard 31.42, for those who at the
start of the year had turned 18 but not 23 (socialavgiftslagen
(2000:980), ch. 2). The government's own account puts it at 1 April 2021
to 30 March 2023 (Prop. 2025/26:66), which is the dating the paper uses.

Its WITHDRAWAL in March 2023 raised the cost of the youngest workers in
the middle of the panel window, and it falls inside the period over which
...
the rival's prediction, not ours. A synthetic world in which only the
payroll withdrawal operates returns -0.015 (0.015) at 24-25: negative,
and not a rejection of the rival.
...
IN THE PAPER
Section 3 (the decline among those aged 24 and 25, whom the reduction
never covered, is about twice that among those aged 22 and 23) and Online
Appendix III.2 (-0.026, SE 0.012, for the covered half and -0.050, SE
0.011, for the half never covered, against the thirteen months before the
adoption window; the covered half at +0.026 through 2023).
...
def load_fine(years) -> pd.DataFrame:
    """
    The fine-banded counts, pulled once and cached.

    THE CONNECTION IS OPENED ONLY IF A YEAR IS MISSING. The first run of
    this script needs a slot that may issue SQL; every rerun reads the
    caches and needs nothing, so a killed job can be resubmitted without a
    database connection and still finish.
    """
    out, conn = [], None
    for y in years:
        ...
        if c is None:
            if conn is None:
                print("    no cache for at least one year; opening a "
                      "connection")
                conn = mc.connect()
        ...
        out.append(c)
    if conn is None:
        print("    every year read from cache; no SQL issued")
...
def main() -> int:
    ...
    fine = load_fine(s61.PANEL_YEARS)
