#!/usr/bin/env python3
"""
90_wfh_margins_adoption.py: which margin each score carries, and whether
measured adoption can replace measured exposure on both sides.
...
======================================================================
  ...
  first stage already does, and so needs a database connection.
======================================================================
...
this script where a joint model is the right instrument, because the
...
script. Nothing is pre-committed here, because an existence check cannot
be pre-committed.

INPUTS AND OUTPUTS
Reads the flows cache and L_baseline_2019* (script 82) for Part C; for Part D, the INFORMATION_SCHEMA catalogue and
...
IN THE PAPER
Nothing from this script is quoted. It ran in the same job as script 89,
its exports are kept beside 89's, and it is shipped so that the job can
be re-run as it ran.
"""
...
def main():
    ...
    # The two scores come from 89's builder, so the script cannot end up
    ...
    # 82's own loader and its own year set, so this script cannot read a
