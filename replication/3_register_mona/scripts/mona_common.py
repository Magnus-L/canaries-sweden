...
outside MONA: the database driver is not loaded and the SQL functions
raise, so the panel builders and the R wrappers can be exercised on
synthetic data.
...
# The directory is disposable and is deleted once the exports are out and
# verified.
...
def runlog(script: str, rc: int, minutes: float):
    ...
    try:
        ...
        line = "%s | %-12s | %-34s | exit %d | %5.1f min\n" % (
            ...
            rc, minutes)
        # Several concurrent jobs append to this one file. A single small
        # append is atomic enough on SMB, but the handle can be briefly
        # locked by another job; retry rather than silently lose the row.
