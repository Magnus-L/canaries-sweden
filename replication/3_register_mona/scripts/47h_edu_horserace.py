...
QUESTION
An alternative to occupation codes is to score exposure from education:
the occupation register is published with a lag, while the education
register records every worker. Which rule should turn an education into
an exposure score, and which rule should assign a worker to an
education? This script builds the score books and tests every candidate
the same way: by re-estimating on years where the truth is observable,
with the education register truncated as it will be truncated for 2024
and 2025. The paper reports the occupation route (script 82); the
education route built here is its comparison arm, and the education
records pulled here divide the sample in the cuts by field of education.
...
IN THE PAPER
No estimate of this script is quoted. Its role is to build inputs: the
education key and score files (load_key, load_scores), the 2019 weights
and the yearly education records (the caches edu_hr_weights_YYYY and
edu_hr_YYYY) and the OL_daioe score book (ScoreBook). Scripts 76, 80,
87, 88 and 89 read the education records to cut the sample by field of
education (Online Appendix III.2, Tables A15 to A17); scripts 61 to 78
use the score book for the education route, which script 83 prints
beside the occupation route for comparison. The estimates in
horserace_estimates.csv concern worker-level education designs that the
