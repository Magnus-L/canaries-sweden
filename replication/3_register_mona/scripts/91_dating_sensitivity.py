#!/usr/bin/env python3
"""
91_dating_sensitivity.py: does the headline depend on where the adoption
window is judged to begin?
...
So the boundary does not sit on a break, and the question is what
...
GRADUAL world of the synthetic test of this script reproduces exactly
that, a spread of 0.013 across the five boundaries on a step of -0.156. So an invariant sweep is
...
One standard error is not an equivalence margin, overlapping intervals do
not establish equivalence, and a formal test would need the cross-fit
covariance, which this run does not produce. Nothing here is reported as
a test.
...
THE SWEEP IS NOT THE PRIMARY ANSWER. Online Appendix Table A12 reports
contrasts taken from the flexible quarterly path of script 84 on periods of equal length and
...
INPUTS AND OUTPUTS
Reads the L_counts caches script 47L writes and the occupation cascade
...
IN THE PAPER
Section 3 (moving the boundary anywhere between July 2023 and July 2024
changes the estimate by at most 0.002) and Online Appendix III.2 (the
ranges -0.041 to -0.039 at 22-25 and -0.050 to -0.040 at 26-30).
...
# The gate. These are the estimates the paper prints, from the export of
# scripts 82 and 83 (occ_route_headline.csv and occ_rest_window.csv).
# Four decimals, as stated in the docstring.
GATE = {
