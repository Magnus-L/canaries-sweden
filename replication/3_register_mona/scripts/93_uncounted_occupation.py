...
THE QUESTION
Online Appendix Table A33 reports the share of employer-declaration
person-months whose worker reaches none of the 2023, 2021 and 2019
individual registers, for all employers and for the top exposure quartile
against the lower three. Script 79's Part C computes these counts with
the 2019 EDUCATION-mix quartiles; this script re-cuts the same counts on
the 2019 OCCUPATION-mix quartiles the paper treats on.
...
OUTPUT (output_93/)
  uncounted_share.csv   same columns as 79's, so the table builder reads
                        either
...
# run leaves no stray output_79 behind.
...
# script 79's summary (Part C) and in the appendix.
...
# route (Online Appendix Table A9, Panel A). Used only for the bound.
