...
IN THE PAPER
Online Appendix III.2, Table A14 and the paragraph beside it: the
...
gain is at least as large on teleworkability as on AI exposure. Table A14
is built from robustness_gradient.csv by the exhibit builders in
4_exhibits/.
"""
...
# The launch is the date of the submitted design; January 2024 is where
# Statistics Sweden's survey records AI use as common (script 61). Both are
...
def main():
    ...
    # employment, so both are estimated rather than assumed. The flow panel is built once per
