...
exercises that rest on it: standard errors clustered by industry, and
industry by age by month effects.
...
Scripts 73, 78 and 79 gave each unmatched firm a cluster of its own, so
their industry-clustered runs report 5,545 and 9,368 clusters against
only 265 three-digit groups: nineteen clusters in twenty are a single
firm. That inference is neither industry clustering nor employer
clustering but a hybrid of the two, and this script replaces it.
...
B. THE CLUSTERING REDONE. The industry-clustered standard errors of
   ...
   before, and expect that to bear on the 22 to 25 step, which script 78
...
IN THE PAPER. The fits this script runs itself are on the education-route
score and are not quoted. Script 83 imports industry_key, part_b, part_c,
MATCH_DP and PRIOR_DIRS and runs Parts B and C on the occupation-route
score: the industry-clustered standard errors of Table 1 and Online
Appendix Table A21, and the retained shares of the industry test in
Section 3 and Table A20. Script 82 imports panel_employers and size_rows
for the sample description of Online Appendix I.2.
"""
...
# Where scripts 78 and 79 wrote the runs this script re-infers. Listed
...
def prior(name: str) -> Path | None:
    """The first of the export folders of scripts 78 and 79 that holds a file."""
    for d in PRIOR_DIRS:
...
def read_prior(fname: str, band: str | None, terms: list):
    """
    A coefficient table from script 78 or 79, if it is on the share.
...
def _se(g, term) -> float | None:
    ...
    Script 78's cluster_industry.csv names the employer-clustered column
    se_employer and its gender_eq2.csv names it se, and a refit here
...
def _coef(g, term) -> float:
    ...
    Script 78's cluster_industry.csv carries the industry run's coefficient
