...
exposure quartile, estimated on script 82's occupation-route score.
...
======================================================================
  ...
  (default output_83); master.py sets both. Part A also needs a
...
QUESTION
Script 82 scores the employer from the occupations its own incumbents
held in 2019 and estimates the headline, the age profile, the margins and
the sex split. Everything else in the paper that uses the exposure
quartile is estimated here on the same score: the first stage against
reported AI use, the descriptive counterpart of the headline, the
reference-window specification, the pre-launch drift, the industry
standard errors, the industry-by-age-by-month test, the credit test and
the firm-size robustness, so that no published number mixes two
...
Part D (part_d). The firm-size robustness. A firm's exposure is a MEAN over its incumbents aged 31 to 69, so
...
floor never protected against this, and script 82's Part A shows it
barely binds at all, 271,047 employers at a floor of one against 262,089
at five.
...
80's Part B reads script 78's exports to get the employer-clustered run
it checks itself against. Those exports are the EDUCATION route's, so read
...
anything from this script or from script 82 may be quoted at all, and its
verdict is stated at the TOP of the summary whichever way it falls.
...
IN THE PAPER
Part A: the first stage (Section 2, 20.9 points in the 2023 firm survey;
Online Appendix III.2 and Figure A5) and the descriptive counts of Online
Appendix III.1. Part B: Table 1's level after adoption, pre-launch drift
and industry standard errors; Online Appendix Tables A19 (drift), A21
(clustering) and A11 (the window). Part C: the industry and credit tests
(Section 3, Table A20). Part D: the precision of the firm score (Online
Appendix III.3, Table A22).
"""
...
def load_modules():
    ...
    exports rather than in the folder of an earlier run.
    """
    ...
    # finds in an earlier run's folder. Those folders hold the EDUCATION
...
def part_b_cluster(counts, sexcounts, occ, key, s61, s67, s73, s78, s80,
    ...
    panel (see load_modules), the check is against this script's own numbers
    ...
    # one name per table, and this script's name is the one the exhibit
    # builders read, so the duplicate goes.
...
def part_c_credit(counts, occ, s61, s73, j47) -> list:
    ...
    adoption step is the quantity script 82 estimated, so the two are a
    check that this script and that one are fitting the same panel.
...
def main():
    ...
    # THE SCORE. Script 82's, built in script 82's own function, and not
