#!/usr/bin/env python3
"""
01_postings_accounting.py: the advertisement sample accounting, 2020 to 2025.

WHAT IT BUILDS
One pass over the annual archives 2020 to 2025 classifies every line in the
order the sample filter applies: parse error; occupation field absent; code
present but not a four-digit SSYK 2012 code; publication date absent or
unparseable; publication year outside 2006 to 2026; advertisement identifier
already seen (in this or an earlier year); kept. Because the order is the
filter's, the kept count reproduces the analysis sample by construction.
Every dated advertisement is also counted by month and by source system (the
channel through which it reached Platsbanken), with and without a valid code,
which gives the coverage of the occupation field over time and by channel;
repeats are counted there, since coverage is a property of the data and
removing repeats an analysis choice. classify_ad is imported by 03, 10 and 17
so that the 2026 quarters are classified identically.

INPUTS   config.JOBADS_DIR/<2020..2025>.jsonl.zip (the 1 per cent samples
         with --sample)
OUTPUTS  output/results/postings_accounting.csv, postings_coverage_monthly.csv;
...
from config import PLATSBANKEN_YEARS, RESULTS, TABLES  # noqa: E402


def classify_ad(ad: dict) -> tuple[str, dict | None]:
    """
    The filter of 1_data_public/02_process_platsbanken.py (extract_ad_fields),
    returning the reason an advertisement is dropped instead of None. The
    order of the checks is the same, so the kept sample is identical by
    construction.
    """
...
def account_year(year: int, seen_ids: set, sample: bool, monthly: dict) -> dict:
    zpath = (config.JOBADS_DIR / f"{year}_sample.jsonl.zip" if sample
             else config.platsbanken_zip(year))
    if not zpath.exists():
        zpath = config.JOBADS_DIR / f"{year}_sample.jsonl.zip"
    if not zpath.exists():
...
def main():
    ...
    print("Advertisement accounting, 2020 to 2025" + (" [SAMPLE]" if args.sample else ""))
    seen: set = set()
    monthly: dict = {}
    ...
    out = RESULTS / f"postings_accounting{suffix}.csv"
    ...
    mout = RESULTS / f"postings_coverage_monthly{suffix}.csv"
    ...
    if not args.sample:
        ...
        (TABLES / "postings_accounting.tex").write_text("\n".join(lines))
        print(f"Saved {TABLES / 'postings_accounting.tex'}")
