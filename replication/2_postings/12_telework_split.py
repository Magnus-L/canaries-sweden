#!/usr/bin/env python3
"""
12_telework_split.py: the post-launch posting coefficient in teleworkable and
non-teleworkable occupations.

WHAT IT ESTIMATES
The Dingel and Neiman (2020) teleworkability classification is carried from
O*NET-SOC to SSYK 2012: O*NET-SOC codes are truncated to SOC 2010 and averaged,
mapped to ISCO-08 with the BLS crosswalk and to SSYK 2012 with Statistics
Sweden's key, averaging with equal weights at each many-to-many step.
Occupations are split at the median score, and specification (2) of the
submitted version, ln(postings + 1) on occupation and month fixed effects,
PostRB x High and PostGPT x High, is estimated in each half and on all
occupations by OLS (statsmodels), standard errors clustered by occupation.

THE WINDOW
The panel is the matched panel of the submitted version (October 2019 to
February 2026, data/processed/postings_daioe_merged.csv, 26,672
occupation-months), which is the one the printed figure was drawn from.

INPUTS   data/raw/dingel_neiman_telework.csv, isco_soc_crosswalk2.xls,
         ssyk2012_isco08.xlsx; data/processed/postings_daioe_merged.csv,
         daioe_quartiles.csv
...
def load_dingel_neiman() -> pd.DataFrame:
    ...
    """
    local = config.DINGEL_NEIMAN_CSV
...
def main():
    print("=" * 70)
    print("Equation (1) split by teleworkability (Dingel and Neiman 2020)")
    print("=" * 70)
    ...
    out_map = RESULTS / "telework_ssyk_mapping.csv"
    ...
    # Save results
    out_res = RESULTS / "telework_did_results.csv"
