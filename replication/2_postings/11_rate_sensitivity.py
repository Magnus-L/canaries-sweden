#!/usr/bin/env python3
"""
11_rate_sensitivity.py: revealed sensitivity to the rate cycle, against AI
exposure.

WHAT IT MEASURES
The seven months between the Riksbank's first rate rise (April 2022) and the
launch of ChatGPT (30 November 2022) give each occupation a revealed
sensitivity to monetary policy that no generative-AI effect can contaminate:
the log change in mean monthly postings (plus one) from January to March 2022
to May to November 2022, for occupations with at least three advertisements a
month in the first window. Negative values mean postings fell. The measure is
related to the DAIOE generative-AI percentile by OLS and by least squares
weighted by posting volume (the weights are each occupation's advertisements
over the whole panel), with the correlation and its p-value for each; a box
plot by quartile with a Kruskal-Wallis test is drawn beside it. Brynjolfsson
et al. (2025) make the same comparison for the United States on the Zens et
al. (2020) measure.

THE WINDOW
The panel is the matched panel of the submitted version (October 2019 to
February 2026, data/processed/postings_daioe_merged.csv), which is the one
the printed figure was drawn from. Only the months of 2022 enter the
sensitivity measure; the window matters for the posting-volume weights alone.

INPUTS   data/processed/postings_daioe_merged.csv, daioe_quartiles.csv (1_data_public/04)
...
PROCESSED, FIGURES, RESULTS = config.PROCESSED, config.FIGURES, config.RESULTS
RIKSBANKEN_HIKE, CHATGPT_LAUNCH = config.RIKSBANKEN_HIKE, config.CHATGPT_LAUNCH
...
def plot_boxplot(df: pd.DataFrame):
    ...
    plt.tight_layout()
    out = FIGURES / "figA_rate_sensitivity_boxplot.png"
    fig.savefig(out, dpi=300)
    plt.close()
    print(f"  Saved: {out.name}")
...
def save_statistics(df, slope_uw, intercept_uw, r_uw, p_uw, se_uw, r_w, p_w):
    ...
    out1 = RESULTS / "rate_sensitivity_correlation.csv"
    stats_df.to_csv(out1, index=False)

    out2 = RESULTS / "rate_sensitivity_by_quartile.csv"
...
def main():
    print("=" * 70)
    print("Revealed rate sensitivity against AI exposure")
    print("=" * 70)
