#!/usr/bin/env python3
"""
14_within_employer.py: the within-employer design on public advertisements,
January 2021 to June 2026.

THE QUESTION
From January 2021 about 99 per cent of Platsbanken advertisements carry the
employer's organisation number, recorded at publication, so the register
design's comparison inside the employer can be run on public data, with no
occupation register and no register lag: do an employer's advertisements in
exposed occupations fall relative to its other advertisements after the
launch, and differently for entry-level advertisements?

DESIGN
Unit: distinct advertisements by employer, exposure quartile and month, from
the employer-by-month-by-occupation counts in config.FIRM_CUBE (see the pack
README for how they are built). Sample: employers with at least five distinct
advertisements over the window that advertise in both an exposed (Q4) and a
less exposed occupation; the panel is balanced and zero-filled over employer
by quartile by month. Specification: Poisson pseudo-maximum likelihood with
PostRB x High (from April 2022) and PostGPT x High (from December 2022) under
employer-by-quartile and employer-by-month effects, standard errors clustered
by employer; and the half-year event study, reference the first half of 2022.
The window before the rate rise is fifteen months, since organisation numbers
begin in January 2021. Variants: (a) all identifying employers; (b)
excluding staffing agencies (SNI 78, from Statistics Sweden's business
register) and public employers (organisation numbers beginning with 2); (c)
entry-level advertisements only, flagged by keywords in the advertisement
text (nyexaminerad, nyutexaminerad, junior, trainee or traineeprogram, ingen
erfarenhet, utan erfarenhet, utan tidigare erfarenhet).

With --variants: (d1, d2) the floor and the screen computed on January 2021
to March 2022 only, so that nothing after the rate rise decides membership,
keeping every quartile cell or only the quartiles used before the rise; (e)
d1 without staffing agencies and public employers; (f) all advertisements and
entry-level advertisements on the employers that pass both screens; (g) the
entry-level differential in one panel of employer by quartile by entry flag
by month, with employer-by-quartile-by-entry and employer-by-entry-by-month
effects and the triple interaction PostGPT x High x Entry. The share of
advertisements the entry rule flags, by year and quartile, is written too.

INPUTS   config.FIRM_CUBE (firm_month_v2.csv.gz), config.SCB_BULK
         (scb_bulkfil.zip); data/processed/daioe_quartiles.csv
...
CUBE = config.FIRM_CUBE
SCB_BULK = config.SCB_BULK
...
def build_panel(cube, daioe, outcome="ads", drop_staffing_public=False,
    ...
    if drop_staffing_public:
        ...
        # cast to bool before negating, or ~ would act bitwise on integers
...
def _tex(rows_df, trip):
    ...
    labels = [("d1_pre_eligible_all_quartiles", "Eligibility before the hike, every quartile cell"),
              ...
              ("e_pre_eligible_excl_staffing_public", "As the first row, excluding staffing agencies and public employers"),
...
def main_variants():
    print("Within-employer design, variants: eligibility before the hike; entry-level on a common sample")
    cube = load_cube()
    ...
    out.to_csv(config.RESULTS / "firm_within_did_variants.csv", index=False)
    audit = entry_audit(cube, daioe)
    audit.to_csv(config.RESULTS / "firm_within_entry_audit.csv", index=False)
    print("  entry-level rule:", ENTRY_RULE)
    print(audit.to_string(index=False))
    (config.TABLES / "firm_within_variants.tex").write_text(_tex(pd.DataFrame(rows), trip))
    print("Saved firm_within_did_variants.csv, firm_within_entry_audit.csv, firm_within_variants.tex")


def main():
    print("Within-employer posting design on public advertisements, January 2021 to June 2026")
    cube = load_cube()
    daioe = pd.read_csv(config.PROCESSED / "daioe_quartiles.csv",
                        dtype={"ssyk4": str})
    daioe["ssyk4"] = daioe["ssyk4"].str.zfill(4)
    ...
    pd.DataFrame(rows).to_csv(config.RESULTS / "firm_within_did.csv",
                              index=False)
