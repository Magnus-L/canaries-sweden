#!/usr/bin/env python3
"""
07_event_study.py: the posting event study and its pre-tests, January 2020 to
June 2026.

WHAT IT ESTIMATES
  Monthly event study: ln(postings) on month dummies interacted with High,
    occupation and month fixed effects (linearmodels PanelOLS), standard
    errors clustered by occupation, February 2020 omitted; a joint Wald test
    on every coefficient before April 2022.
  Quarterly event study: the same on occupation-by-quarter cells, 2020Q1
    omitted; a joint Wald test on every quarter before 2022Q2.
  The average post-launch coefficient (December 2022 onwards), its
    delta-method standard error, and a simplified sensitivity bound, the
    interval theta +/- (1.96 SE + Mbar x Dmax), where Dmax is the largest
    month-to-month change in the pre-April-2022 coefficients. The simplified
    bound is narrower than the exact Rambachan and Roth interval, since it
    allows one Dmax of bias for the whole post-period average; it is written
    out but not reported. The paper's panel (b) is drawn from the exact
    bounds of 08 by 09.
  The full coefficient vector and its clustered covariance are written for 08.

CHECK
Run on January 2020 to December 2025, the script reproduces the submitted
version's numbers (chi2(26) = 107.059, chi2(8) = 52.769, theta = -0.16897,
SE 0.05905) and stops if it does not, before estimating on the window to June
2026. It also stops unless the window to June 2026 holds 28,084 cells, the
sample of Equation (1) in 03.
...
from _common import (config, set_rcparams, save_pdf_png,  # noqa: E402
                     DARK_BLUE, ORANGE, TEAL, GRAY)

set_rcparams()

PANEL = config.PROCESSED / "postings_daioe_merged_extended.csv"
TAB = config.RESULTS
START, BASE_MONTH = "2020-01", "2020-02"
RB, GPT = config.RIKSBANKEN_HIKE[:7], config.CHATGPT_LAUNCH[:7]
...
def analyse(end):
    ...
    # The simplified bound: Dmax over the pre-April-2022 coefficients, the
    # reference month included as zero.
    ...
    # for the exact HonestDiD computation of 08.
...
def plot_rr(grid, theta, mbar_bd):
    ...
    fig.tight_layout()
    save_pdf_png(fig, "figA6_rambachan_roth_simplified_v3")
    plt.close(fig)


def main():
    print("Posting event study, January 2020 to June 2026")
    ...
    for k, v in OLD.items():
        ...
        assert abs(got[k] - v) < tol * max(1, abs(v)), \
            f"CHECK: the window to December 2025 does not reproduce {k}: {got[k]} vs {v}"
    print("  check passed: January 2020 to December 2025 reproduces the submitted version's numbers")
