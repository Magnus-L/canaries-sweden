...
Stockholm 30, the monthly mean of daily closes indexed to 100 at February
2020; the lower panel is Platsbanken postings by DAIOE generative-AI exposure
quartile on the same base, as a three-month centred moving average, to June
2026. Series are told apart by line style and a legend, and the two event
lines (the Riksbank's first rate rise, April 2022; the ChatGPT launch,
November 2022) carry their dates.

Figure A1 (figA_posting_context), four panels: (a) the United States, the
S&P 500 against the Indeed Hiring Lab total-postings index; (b) Sweden, the
OMXS30 and the OMX Stockholm All-Share against Platsbanken postings on one
indexed axis; (c) the four exposure quartiles drawn one by one; (d) the
Riksbank policy rate, its tightening cycle shaded and the seven months to
the ChatGPT launch marked.

Figure A9 (fig3_firm_entry_es): the half-year Poisson event-study
coefficients of the within-employer posting design of 14, all
advertisements and entry-level advertisements, reference the first half of
2022, with 95 per cent intervals.

INPUTS   data/processed/omxs30_monthly.csv, omxspi_monthly.csv,
         riksbank_monthly.csv (1_data_public/03); data/raw/sp500_daily.csv,
         indeed_us_aggregate.csv; output/results/
...
from _common import config, save_pdf_png  # noqa: E402
from config import (PROCESSED, RAW, RESULTS, FIGURES,  # noqa: E402
                    POSTINGS_DESCRIPTIVE_END, POSTINGS_REGRESSION_END,
                    BASE_MONTH, RIKSBANKEN_HIKE, CHATGPT_LAUNCH,
...
def fig1_two_panel():
    ...
    # The quartile series to June 2026 (03) when it has been built
    ext = RESULTS / "postings_quartile_indexed_extended.csv"
    if ext.exists():
        q = pd.read_csv(ext)
    ...
    fig.savefig(FIGURES / "fig1_two_panel.pdf", bbox_inches="tight")
    fig.savefig(FIGURES / "fig1_two_panel.png", dpi=300,
                bbox_inches="tight")
    plt.close(fig)
...
def fig3_firm_entry_es():
    """The within-employer posting event study of Online Appendix V (14)."""
    src = RESULTS / "firm_within_es.csv"
    if not src.exists():
        print("  Figure A9: run 14_within_employer.py first")
        return
    print("  Figure A9: within-employer event study")
    es = pd.read_csv(src)
    ...
    ax.spines[["top", "right"]].set_visible(False)
    fig.savefig(FIGURES / "fig3_firm_entry_es.pdf", bbox_inches="tight")
    fig.savefig(FIGURES / "fig3_firm_entry_es.png", dpi=300,
                bbox_inches="tight")
    plt.close(fig)
...
def _quartile_series():
    """Postings by exposure quartile, to June 2026 when 03 has run.

    The submitted version's series ends with two months from the live feed,
    so it is cut at POSTINGS_DESCRIPTIVE_END when it is the only one
    available.
    """
    ext = RESULTS / "postings_quartile_indexed_extended.csv"
    if ext.exists():
...
def figA_posting_context():
    ...
    quartiles move together, and what the monetary cycle behind them
    looked like.
    """
    print("  Figure A1: posting context, four panels")
    ...
    save_pdf_png(fig, "figA_posting_context")
    plt.close(fig)
    print(f"    saved figA_posting_context.pdf/.png, postings to {cut:%Y-%m}")


def main():
    print("Figures of the posting margin")
    fig1_two_panel()
    fig3_firm_entry_es()
