...
**A1 is the gatekeeper. Do not submit to Economics Letters without it.**

1. **Rambachan-Roth sensitivity analysis** for the employment event study (ALL age groups, but at minimum ages 22-25 and 26-30). Use the `honestdid` R package or implement in Python following Rambachan & Roth (2023, RES). Report the breakdown value $\bar{M}$ and honest confidence intervals under **relative magnitudes** restrictions. **Why this is critical:** pre-trend tests reject at p=0.000 for all six age groups, and pre-period magnitudes (-0.016 to +0.013) are comparable to the average DiD coefficient (-0.010 for ages 22-25). Three simulated referees and an independent adversarial review all identified this as the #1 concern. If the employment results survive honest CI under plausible trend violations, the paper's contribution is secure. If they do not, we must reframe before submitting.

2. **Poisson QMLE robustness**: re-estimate the main DiD (Equation A1) and event study using Poisson quasi-maximum likelihood instead of OLS on ln(count+1), following Chen & Roth (2024, QJE). Also report results **excluding zero-employment cells** to confirm the intensive margin alone shows the pattern. This addresses the concern that zero-filled cells + ln(y+1) produces estimates driven by small cells going to zero (idiosyncratic turnover rather than systematic displacement). Script 15 has a pyfixest Poisson fallback — adapt for the balanced panel.

3. **Employment summary statistics** for appendix Table A2: N employers, N employer×quartile cells, mean/median cell sizes, **share of zero-employment cells by age group × quartile × half-year period**, and **how the zero-cell share changes over time**. Referees will use this to assess whether results are driven by extensive-margin zeros or intensive-margin declines in larger firms.

4. **Placebo treatment date test**: re-estimate the employment event study with a fake "PostGPT" cutoff at **November 2021** (one year early) and the same reference period. If no acceleration appears at the placebo date, this confirms the post-2024 pattern is not a pre-existing trend continuing through treatment. This is the simplest and most convincing defence against the "delayed drift" critique. Also test with a **July 2022** placebo (between Riksbank and ChatGPT).

5. **Alternative reference period (2021H2)**: Script 18 already computes this. Export the event study coefficients CSV and **include the figure in the appendix** (not just state "qualitatively unchanged"). This addresses the concern that the 2022H1 reference period is contaminated by the Riksbank tightening cycle start (April 2022). The significant γ₁ for ages 22-25 makes this reference period choice consequential.

6. **Spotlight figures** for payroll administrators (SSYK 4112) and receptionists (SSYK 4225) — same format as software developers and customer service figures.

### Priority B — Should complete (raised by 2+ simulated referees)

7. **Teleworkability split for employment** (not just postings): crosswalk Dingel-Neiman teleworkability to SSYK and re-estimate the employer-level DiD + event study separately for teleworkable vs non-teleworkable occupations. If the employment age gradient also concentrates in non-teleworkable occupations, this strongly corroborates the main result.
8. **Alternative SE clustering**: report SEs clustered at (a) the employer level and (b) the occupation level, in addition to the current employer×quartile clustering. Present as a robustness table. With 11-30M observations and potentially millions of employer×quartile clusters, the effective number of independent treatment-group comparisons is much smaller than the cluster count suggests.
9. **SSYK attrition robustness**: re-estimate the employment DiD + event study restricting the sample to **2019-2023 only** (where SSYK coverage is strongest, 90%+). The non-match rate rises to 20% in 2025, precisely when event study effects are largest. The appendix currently claims "core results are strongest in 2023" but this contradicts the event study (near-zero in 2023, large in 2024-2025). This analysis resolves that contradiction.
10. **Employment age gradient with Eloundou measure**: re-estimate the employer-level DiD using the Eloundou et al. (2024) GPT exposure score instead of DAIOE. A referee noted that the posting timing test is measure-dependent (DAIOE loads effect onto Riksbank period; Eloundou loads onto ChatGPT period). Showing the employment result holds across measures would address this.
11. **Composition entrant share over time**: report the share of young entrants (first observed in AGI) entering Q4 occupations, **by year** (2019-2025). The current "13% in 2023" statistic does not rule out a composition channel — the relevant test is whether this share *changes differentially* after ChatGPT. If the Q4 share among young entrants was 18% in 2021 and fell to 13% by 2023, that IS a composition shift. Report the full time series.
12. **Cumulative pre-trend bias calculation**: for each age group, compute the per-half-year pre-trend slope from the event study coefficients and multiply by the number of post-treatment half-years. Report alongside the actual post-treatment endpoint. This gives the reader a concrete sense of how much of the post-period effect could be explained by linear extrapolation of pre-trends. (E.g., if the pre-trend drift is -0.003/half-year and there are 6 post-treatment half-years, cumulative bias = -0.018, vs the observed endpoint of -0.055 for ages 22-25.)
