#!/usr/bin/env python3
"""
28_mona_ssyk_attrition.py -- SSYK attrition robustness: restrict sample to 2019-2023.
...
WHY THIS SCRIPT EXISTS:
  The SSYK non-match rate rises from ~10% in 2019-2023 to ~15% in 2024
  and ~20% in 2025 (because LISA ends at 2023 and new entrants lack
  SSYK codes). The event study effects are largest precisely in 2024-2025,
  raising the concern that results are driven by differential SSYK attrition
  across AI-exposure quartiles rather than genuine employment shifts.

  This script re-estimates the employer-level DiD + event study restricting
  the sample to 2019-2023 ONLY (where SSYK coverage is strongest, 90%+).
  If the effect is visible even in 2023 (where SSYK coverage is good),
  the 2024-2025 acceleration is less likely to be an artifact.
...
SPECIFICATION:
  ...
  Only change: years pulled = 2019-2023 (not 2019-2025).
  Reference period: 2022H1 (same as main specification).
  Post-treatment periods: 2022H2, 2023H1, 2023H2.

OUTPUT FILES:
  1. ssyk_robust_es_all.csv          -- event study coefficients (2019-2023)
  2. ssyk_robust_did_results.csv     -- DiD coefficients
  3. ssyk_robust_pretrends.txt       -- pre-trend joint F-test results
  4. ssyk_robust_es_*.png            -- figures per age group
  5. ssyk_robust_summary.txt         -- comparison with full-sample results
"""
...
# ======================================================================
#   STEP 1: LOAD DATA (same pipeline as script 18, but 2019-2023 only)
# ======================================================================
...
def load_and_prepare():
    ...
    KEY DIFFERENCE: Only pulls years 2019-2023 (not 2024-2025).
    This restricts the sample to the period where SSYK coverage
    is strongest (90%+ match rate).
    """
    print("=" * 70)
    print("STEP 1: Loading and preparing data (2019-2023 ONLY)")
    print("=" * 70)

    # Pull year by year -- RESTRICTED TO 2019-2023
    frames = []
    t0 = time.time()
    for year in range(2019, 2024):  # <-- KEY CHANGE: 2019-2023 only
...
# ======================================================================
#   STEP 2: CORRECTED EVENT STUDY (2019-2023 only)
# ======================================================================
...
def run_corrected_event_study(agg, ref_halfyear=None):
    ...
    print(f"  SAMPLE RESTRICTION: 2019-2023 only (SSYK attrition robustness)")
    print("=" * 70)
    ...
    if all_es_results:
        ...
        fname = f"ssyk_robust_es_all{suffix}.csv"
        es_df.to_csv(OUTPUT_DIR / fname, index=False)
        print(f"\n  Saved event study coefficients -> {fname}")

    if pretrend_tests:
        pt_df = pd.DataFrame(pretrend_tests)
        pt_df.to_csv(OUTPUT_DIR / f"ssyk_robust_pretrends{suffix}.csv", index=False)
        pt_path = OUTPUT_DIR / f"ssyk_robust_pretrends{suffix}.txt"
        pt_df.to_string(pt_path, index=False)
        print(f"  Saved pre-trend tests -> ssyk_robust_pretrends{suffix}.txt")
...
def plot_event_studies(es_df):
    ...
    for age_label in AGE_GROUPS:
        ...
        ax.set_title(f"Event study: {age_label} (2019-2023, SSYK robustness)",
                     fontsize=13)
        ax.legend(fontsize=9)

        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / f"ssyk_robust_es_{age_label.replace('+','plus')}.png",
                    dpi=150)
        plt.close(fig)
    ...
    fig.suptitle("SSYK attrition robustness: 2019-2023 only "
                 "(employer x quartile + employer x month FE)",
                 fontsize=13, y=1.01)
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / "ssyk_robust_es_panel.png", dpi=150)
    plt.close(fig)
...
def run_did(agg):
    ...
    Same specification as script 15 Step 2, but on the 2019-2023 sample.
    Post = 2022H2 onwards (after ChatGPT release in 2022-12, but the
    Riksbank rate hike in 2022-04 is also relevant).
    """
    from linearmodels.panel import PanelOLS
    ...
    if did_results:
        ...
        did_df.to_csv(OUTPUT_DIR / "ssyk_robust_did_results.csv", index=False)
        print(f"\n  Saved DiD results -> ssyk_robust_did_results.csv")
        print("\n  === DiD RESULTS (2019-2023 only) ===")
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("28_mona_ssyk_attrition.py")
    print("SSYK attrition robustness: restrict sample to 2019-2023")
    ...
    # Step 1: Load data (2019-2023 only)
    agg = load_and_prepare()

    # Step 2: Event study (reference = 2022H1)
    ...
    summary = [
        "=" * 60,
        "SSYK ATTRITION ROBUSTNESS SUMMARY",
        "=" * 60,
        f"Script: 28_mona_ssyk_attrition.py",
        f"Sample: 2019-2023 ONLY (SSYK coverage 90%+)",
        f"FE: employer x quartile (entity) + employer x month (other_effects)",
        f"Runtime: {elapsed/60:.1f} minutes",
        "",
        "RATIONALE:",
        "  SSYK non-match rate rises from ~10% (2019-2023) to ~20% (2025).",
        "  If results hold in 2019-2023, the 2024-2025 acceleration is less",
        "  likely to be an artifact of differential SSYK attrition.",
        "",
        "EVENT STUDY PERIODS: 2019H1 through 2023H2",
        f"Reference: {REF_HALFYEAR}",
        "Post-treatment: 2022H2, 2023H1, 2023H2",
        "",
    ...
    summary.append("  Compare these coefficients with full-sample (script 18) results.")
    summary.append("  If signs and significance match, the SSYK attrition concern")
    summary.append("  does not drive the main findings. If effects disappear, the")
    summary.append("  2024-2025 results may reflect differential attrition.")

    summary_text = "\n".join(summary)
    (OUTPUT_DIR / "ssyk_robust_summary.txt").write_text(summary_text)
    print(f"\n{summary_text}")
